...
void GameBall::waitFor(int mills) const
{
	// Wait until 16ms has elapsed since last frame
	while (!SDL_TICKS_PASSED(SDL_GetTicks(), mTicksCount + mills));
}

void GameBall::renderRect(int x, int y, int w, int h) const
{
	SDL_Rect rect {
		x,	// Top left x
		y,	// Top left y
